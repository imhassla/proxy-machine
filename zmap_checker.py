...
# Get the initial count of proxies from the database
def get_initial_proxy_count(proxy_type):
    conn = sqlite3.connect('data.db')
    cursor = conn.cursor()
    try:
        cursor.execute(f"SELECT COUNT(*) FROM {proxy_type}")
        initial_count = cursor.fetchone()[0]
    except Exception as e:
        print(f"Error fetching initial proxy count from database: {e}")
        initial_count = 0
    finally:
        conn.close()
    return initial_count

# Get the current count of proxies from the database
def get_current_proxy_count(proxy_type):
    conn = sqlite3.connect('data.db')
    cursor = conn.cursor()
    try:
        cursor.execute(f"SELECT COUNT(*) FROM {proxy_type}")
        ...
        print(f"Error fetching current proxy count from database: {e}")
        current_count = 0
    finally:
        conn.close()
...
def print_status(proxy_type, initial_count):
    while True:
        with lock:
            current_count = get_current_proxy_count(proxy_type)
            successful_proxies = current_count - initial_count
            print(f"threads: {active_checker_threads} | queue: {checker_queue.qsize()} | Total checked addresses: {total_zmap_addresses} | New proxies: {successful_proxies}")
        time.sleep(10)

if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: python zmap_checker.py <proxy_type>")
        sys.exit(1)

    proxy_type = sys.argv[1]
    initial_count = get_initial_proxy_count(proxy_type)

    threading.Thread(target=print_status, args=(proxy_type, initial_count), daemon=True).start()
